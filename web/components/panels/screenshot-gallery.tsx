    ...
    <Card className="p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {shots.map((shot) => (
          ...
          >
            <Image src={shot.url} alt={shot.label} width={1200} height={800} className="h-44 w-full object-cover transition duration-300 group-hover:scale-105" />
      ...
      {active && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-6 backdrop-blur-sm">
          <div className="relative w-full max-w-5xl overflow-hidden rounded-xl border border-white/10 bg-bg">
            ...
            <Image src={active.url} alt={active.label} width={1600} height={900} className="h-auto w-full" />
          </div>
        </div>
      )}
    </Card>
