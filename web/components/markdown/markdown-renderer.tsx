...
const PrismHighlighter = SyntaxHighlighter as unknown as React.ComponentType<{
  style: unknown;
  language?: string;
...
const MarkdownRenderer: React.FC<Props> = ({ content = "", className }) => {
  ...
  return (
    <div
      className={cn(
        "w-full max-w-4xl mx-auto px-6 text-base leading-7 text-foreground",
        className
      )}
    >
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ ...props }) => (
            <h3
              className="mt-6 mb-2 text-xl font-medium leading-snug"
              {...props}
            />
          ),
          ...
          p: ({ ...props }) => (
            <p
              className="mb-6 text-foreground/90 text-base leading-8"
              {...props}
            />
          ),
          ...
          code({ className, children, ...props }) {
            ...
            // If language exists => block code else inline code
            if (match) {
          ...
          ul: ({ ...props }) => (
            <ul
              className="list-disc ml-6 my-6 space-y-2 text-foreground/85"
              {...props}
            />
          ),
          ol: ({ ...props }) => (
            <ol className="list-decimal ml-6 my-4 space-y-1.5" {...props} />
          ),
